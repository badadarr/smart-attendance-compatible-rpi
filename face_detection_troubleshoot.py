...
def test_face_detection_on_image(image_path, face_cascade):
    ...
    for i, config in enumerate(detection_configs):
        ...
        print(f"  Config {i+1}: {len(faces)} faces (scale={config['scaleFactor']}, neighbors={config['minNeighbors']})")
    ...
    try:
        ...
        if camera.isOpened():
            ...
            if ret and frame is not None:
                return test_face_detection_on_frame(frame, face_cascade, "pi_camera_frame.jpg")
        ...
        try:
            ...
            if cap.isOpened():
                ...
                if ret:
                    ...
                    return test_face_detection_on_frame(frame, face_cascade, f"usb_camera_{i}_frame.jpg")
        except:
            continue
